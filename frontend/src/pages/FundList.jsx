...
export default function FundList({ user }) {
  ...
  const fetchFundsData = async (isSilent = false) => {
    ...
    try {
      fetch(
        `http://localhost:5000/api/live/quotes?symbols=RELIANCE.NS,TCS.NS,HDFCBANK.NS,INFY.NS,ZOMATO.NS`,
      )
        .then((res) => res.json())
      ...
      try {
        const wlRes = await fetch(
          `http://localhost:5000/api/watchlist/${currentUserId}`,
        );
      ...
      if (symbolString) {
        const liveRes = await fetch(
          `http://localhost:5000/api/live/quotes?symbols=${symbolString}`,
        );
      ...
      setFunds(
        dbFunds.map((fund) => {
          const liveQuote = liveData.find(
            (q) => q.symbol === fund.ticker_symbol,
          );
          return {
            ...fund,
            current_nav: liveQuote
  ...
  useEffect(() => {
    if (selectedFund) {
      ...
      fetch(
        `http://localhost:5000/api/live/history/${selectedFund.ticker_symbol}`,
      )
        .then((res) => res.json())
        .then((data) => {
    ...
    if (query.length > 1) {
      try {
        const res = await fetch(
          `http://localhost:5000/api/live/search/${query}`,
        );
  ...
  const addToMarketplace = async (stock) => {
    try {
      const res = await fetch("http://localhost:5000/api/funds/add", {
        ...
        body: JSON.stringify({
          fund_name: stock.shortname || stock.symbol,
  ...
  const handleCheckout = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setWalletBalance(data.newBalance); // Instantly update wallet UI!
        showToast(`Successfully purchased ${selectedFund.ticker_symbol}!`);
        setSelectedFund(null); // Close the drawer
      } else {
  ...
  const estimatedShares =
    selectedFund && amount && selectedFund.current_nav > 0
        ...
        <div className="nav-inner">
          ...
          <div className="search-box">
            ...
            <input
              ref={searchInputRef}
              type="text"
              placeholder="Search companies (e.g., ITC.NS)..."
              value={searchQuery}
      ...
      <div className="content-wrapper">
        ...
        <div style={{ marginBottom: "3rem" }}>
          <h2 className="section-title">
            <Activity color="#2563eb" /> Trending Nifty 50
          </h2>
        ...
        {watchlist.length > 0 && (
          <div style={{ marginBottom: "3rem" }}>
            <h2 className="section-title">
              <Star color="#f59e0b" fill="#f59e0b" /> Your Watchlist
            </h2>
              ...
              {watchlist.map((fund) => (
                ...
                >
                  ...
                  >
                    <button
                      onClick={(e) =>
                        removeFromWatchlist(fund.id, fund.fund_name, e)
                      }
                      style={{
                        background: "#fff1f2",
                        color: "#e11d48",
                  ...
                  >
                    <div>
                      ...
                      <span
                        ...
                        className={fund.change >= 0 ? "text-green" : "text-red"}
                      >
                        {fund.change > 0 ? "+" : ""}
                        {fund.change ? fund.change.toFixed(2) : "0.00"}%
                      </span>
                    </div>
                    <button
                      style={{
        ...
        <h2 className="section-title">
          <Activity color="#2563eb" /> Explore Assets
        </h2>
          ...
          <div className="fund-grid">
            {funds.map((fund) => (
              ...
              >
                ...
                >
                  <button
                    onClick={(e) => addToWatchlist(fund, e)}
                    style={{
                      background: "#fef3c7",
                      color: "#d97706",
                  ...
                  <button
                    onClick={(e) => removeFund(fund.id, fund.fund_name, e)}
                    style={{
                      background: "#f1f5f9",
                      color: "#64748b",
                      border: "1px solid #e2e8f0",
              ...
              >
                <div className="drawer-header">
                  <div>
                    ...
                    >
                      {selectedFund.ticker_symbol.replace(".NS", "")}
                    </h2>
                    <p
                      style={{
                        margin: 0,
                  ...
                  <div style={{ marginBottom: "1.5rem" }}>
                    ...
                    >
                      <div>
                        ...
                        >
                          ₹{Number(selectedFund.current_nav).toFixed(2)}
                        </span>
                        <span
                          style={{
                            marginLeft: "0.5rem",
                          ...
                          className={
                            selectedFund.change >= 0 ? "text-green" : "text-red"
                          }
                        >
                          {selectedFund.change > 0 ? "+" : ""}
                          {selectedFund.change?.toFixed(2)}%
                        </span>
                      </div>
                      <span
                        style={{
                      ...
                      {chartLoading ? (
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                          ...
                          >
                            ...
                            {/* ✨ NEW: Clean background grid lines */}
                            <CartesianGrid
                              strokeDasharray="3 3"
                              vertical={false}
                              stroke="#f1f5f9"
                            ...
                            <XAxis
                              ...
                              minTickGap={30}
                              tickFormatter={(str) => {
                                // If it's a real date string (YYYY-MM-DD), format to short month (e.g., "Jan")
                                if (str.includes("-")) {
                                ...
                                return str; // Fallback for the "Week X" generated data
                              }}
                            />

                            {/* ✨ NEW: Y-Axis showing Prices on the Right */}
                            {/* <YAxis
                  ...
                  <form onSubmit={handleCheckout}>
                    ...
                    <div style={{ display: "flex", gap: "12px" }}>
                      ...
                      >
                        {isSubmitting
                          ? "Processing..."
                          : `BUY ${selectedFund.ticker_symbol.replace(".NS", "")}`}
                      </button>
                    </div>
                  </form>
                </div>
